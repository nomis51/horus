...
public class Cli
{
    ...
    private void Destroy()
    {
            ...
            if (choiceSync == Locale.Get("y"))
            {
                ...
                if (resultGitPush.HasError)
                {
                    AnsiConsole.MarkupLine(
                        $"[{GetErrorColor(resultGitPush.Error!.Severity)}]{resultGitPush.Error!.Message}[/]");
                    return;
                }
            }
        }
        ...
        {
            AnsiConsole.MarkupLine(
                $"[{GetErrorColor(resultDestroyStore.Error!.Severity)}]{resultDestroyStore.Error!.Message}[/]");
            return;
        }
    ...
    private void Config()
    {
        ...
        while (true)
        {
            ...
            if (choice == Locale.Get("save"))
            {
                ...
                if (result.HasError)
                {
                    AnsiConsole.MarkupLine($"[{GetErrorColor(result.Error!.Severity)}]{result.Error!.Message}[/]");
                    return;
                }
    ...
    private void Generate(IReadOnlyList<string> args)
    {
        ...
        var resultGenerateNewPassword = AppService.Instance.GenerateNewPassword(
            name,
            length,
            customAlphabet
        );
        ...
        {
            AnsiConsole.MarkupLine(
                $"[{GetErrorColor(resultGenerateNewPassword.Error!.Severity)}]{resultGenerateNewPassword.Error!.Message}[/]");
            return;
        }
        ...
        while (true)
        {
            ...
            if (choice == Locale.Get("quit"))
            {
                ...
                AnsiConsole.MarkupLine(
                    resultEditMetadatas.HasError
                        ? $"[{GetErrorColor(resultEditMetadatas.Error!.Severity)}]{resultEditMetadatas.Error!.Message}[/]"
            ...
            if (choice == Locale.Get("thePassword"))
            {
                ...
                if (choicePassword == Locale.Get("questions.enterPasswordManually"))
                {
                    ...
                    {
                        AnsiConsole.MarkupLine(
                            $"[{GetErrorColor(resultEditPassword.Error!.Severity)}]{resultEditPassword.Error!.Message}[/]");
                        return;
                    }
            ...
            if (choice == Locale.Get("theMetadata"))
            {
                ...
                {
                    AnsiConsole.MarkupLine(
                        $"[{GetErrorColor(resultEditMetadatas2.Error!.Severity)}]{resultEditMetadatas2.Error!.Message}[/]");
                    return;
                }
